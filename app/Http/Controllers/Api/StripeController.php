...
class StripeController extends Controller
{
    ...
    private function handleCheckoutSessionCompleted($session)
    {
        ...
        if (!$orderId) {
            Log::message('No order ID in session metadata', ['session_id' => $session->id]);
        ...
        if (!$order) {
            Log::message('Order not found', ['order_id' => $orderId]);
        ...
        if ($session->payment_status === 'paid') {
            ...
            try {
                ...
                Log::message('message processing completed checkout', [
                    'order_id' => $order->id,
                    'message' => $e->getMessage(),
                    'trace' => $e->getTraceAsString()
                ]);
