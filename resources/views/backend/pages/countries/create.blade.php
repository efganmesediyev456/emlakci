...
@section('content')
    <div class="container">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.countries.store') }}" method="POST" id="saveForm"
                        ...
                        @foreach ($languages as $language)
                            <div class="tab-pane fade show @if ($loop->first) active @endif"
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="mb-3">
                                            <label for="title_{{ $language->code }}" class="form-label">Ölkə adı
                                                ({{ $language->code }})</label>
                                            <input type="text" class="form-control" name="title[{{ $language->code }}]"
                                                id="title_{{ $language->code }}" placeholder="Ölkə adını daxil edin">
                                        </div>
                                    </div>
                                </div>
                            </div>
